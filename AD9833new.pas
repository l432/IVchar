...
uses
  ...
  StdCtrls, IniFiles, OlegShowTypes, ArduinoDeviceNew, AD9833, 
  ArduinoDeviceShowNew;
...
type
 ...
 TAD9833New=class(TArduinoSetterNew)
  private
...
TAD9833ShowNew=class(TArduinoSetterShowNew)
 ...
 public
  Constructor Create(AD9833:TAD9833new;
                     CPL:TPanel;
...
procedure TAD9833New.AddWordToSendArray(HighByte:byte;LowByte:byte=$00);
begin
...
function TAD9833New.ConvertFreqValueToKod(Freq: double): LongWord;
begin
 Result:=round(min(Freq,AD9833_MaxFreq)*$10000000/25.0e6)and $0FFFFFFF;
end;

function TAD9833New.ConvertPhaseValueToKod(Phase: double): word;
begin
 Result:= round((round(Phase) mod 360)*$1000/360.0)and $0FFF;
end;

constructor TAD9833New.Create;
begin
 inherited Create('AD9833');
end;

procedure TAD9833New.CreateHook;
...
procedure TAD9833New.Generate;
begin
  Action();
end;

procedure TAD9833New.Action;
begin
 SetLength(fData,2);
...
procedure TAD9833New.AddFreqHalfKodToSendArray(FreqLow: Word);
begin
...
procedure TAD9833New.AddWordToSendArray;
begin
...
procedure TAD9833New.PrepareToFreqChange;
...
procedure TAD9833New.PrepareToModeChange;
begin
 if fMode=ad9833_mode_off then
  begin
...
procedure TAD9833New.PrepareToPhaseChange;
begin
...
procedure TAD9833New.PinsCreate;
begin
  Pins := TPins.Create(Name,['Control'],1);
end;

procedure TAD9833New.Reset;
begin
 fMode:=ad9833_mode_off;
 Action();
end;
...
procedure TAD9833New.SetActiveChanel(const Value: TAD9833_ChanelNumber);
begin
  FActiveChanel := Value;
end;

procedure TAD9833New.SetFreq(chan: TAD9833_ChanelNumber; Freq: double);
begin
 fFreq[chan]:=ConvertFreqValueToKod(Freq);
end;

procedure TAD9833New.SetMode(const Value: TAD9833_Mode);
begin
  FMode := Value;
end;

procedure TAD9833New.SetPhase(chan: TAD9833_ChanelNumber; Phase: double);
begin
...
constructor TAD9833ShowNew.Create(AD9833: TAD9833new;
                       CPL: TPanel;
                       PinVariant: TStringList;
                       Fr1Data, Ph1Data, Fr2Data, Ph2Data: TStaticText;
                       Fr1L, Ph1L, Fr2L, Ph2L: TLabel;
...
procedure TAD9833ShowNew.CreatePinShow(PinLs: array of TPanel;
                                    PinVariant: TStringList);
begin
  PinShow:=TOnePinsShow.Create(fArduinoSetter.Pins,PinLs[0],PinVariant);
end;

procedure TAD9833ShowNew.Free;
begin
 fPhaseCh2Show.Free;
 fPhaseCh1Show.Free;
 fFreqCh2Show.Free;
...
procedure TAD9833ShowNew.GenerateButtonClick(Sender: TObject);
begin
 ...
 (fArduinoSetter as TAD9833new).Generate();
end;

procedure TAD9833ShowNew.HookReadFromIniFile(ConfigFile: TIniFile);
begin
...
procedure TAD9833ShowNew.ResetButtonClick(Sender: TObject);
begin
 (fArduinoSetter as TAD9833new).Reset();
 fPhaseCh2Show.ColorToActive(false);
 fPhaseCh1Show.ColorToActive(false);
 fFreqCh2Show.ColorToActive(false);
 fFreqCh1Show.ColorToActive(false);
...
procedure TAD9833ShowNew.WriteToIniFile(ConfigFile: TIniFile);
begin
...
begin
  (fArduinoSetter as TAD9833new).Mode:=TAD9833_Mode(fMode.ItemIndex);
  (fArduinoSetter as TAD9833new).SetActiveChanel(ChanNumber);
  if ChanNumber=0 then
   begin
    (fArduinoSetter as TAD9833new).SetFreq(ChanNumber, fFreqCh1Show.Data);
    (fArduinoSetter as TAD9833new).SetPhase(ChanNumber, fPhaseCh1Show.Data);
   end            else
   begin
    (fArduinoSetter as TAD9833new).SetFreq(ChanNumber, fFreqCh2Show.Data);
    (fArduinoSetter as TAD9833new).SetPhase(ChanNumber, fPhaseCh2Show.Data);
   end;
